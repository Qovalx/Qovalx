 ...
 * Path in repository: /assets/js/khaled-widget.v11.js
 *
 * Framework free. Add one line before </body> in every page layout:
 *   <script defer src="/assets/js/khaled-widget.v11.js"></script>
 *
...
(function () {
  ...
  // Relative on purpose. It resolves to whichever origin served the page, so it
  // is correct on www, on a pages.dev preview and on localhost alike, and the
  // apex redirect is never in the path because the document has already
  // followed it. Naming www absolutely would make every non-www origin a
  // cross-origin request; the endpoint answers preflight for the two canonical
  // origins, but there is no reason to force a preflight at all from here.
  ...
  // The QOVALX mark: a hexagon holding three nodes. Decorative, so it carries
  // aria-hidden and contributes nothing to the accessible name. Built node by
  // node with createElementNS, in keeping with the rest of this file.
  //
  // The three nodes were joined by ivory strokes at 55% opacity, which over the
  // navy header composited to grey and read as a third colour that is not in
  // the palette. They are gone, and all three nodes now carry the hexagon's
  // gold. Geometry is untouched: same centres, same radii, same hexagon path.
  // One array, drawn at both sizes, so the launcher and the panel header cannot
  // drift apart.
  ...
  var MARK = [
    ...
    ["circle", { cx: "11.5", cy: "20", r: "1.8", fill: "#D7A347" }],
    ["circle", { cx: "20.5", cy: "20", r: "1.8", fill: "#D7A347" }]
  ];
  ...
  function injectStyles() {
    var css = [
      ...
      // No box: no border, no fill, no radius. A single gold rule under the
      // text is the whole affordance. Only the label takes this colour: the
      // mark keeps its own, so it is the same drawing as the header copy.
      ".qx-launcher{display:inline-flex;align-items:center;gap:10px;padding:6px 2px;border:0;border-bottom:1px solid #D7A347;background:none;color:#F1E3D8;font:inherit;font-size:12px;letter-spacing:.08em;text-transform:uppercase;cursor:pointer;transition:color .2s ease}",
      ".qx-launcher:hover{color:#D7A347}",
      ".qx-mark{display:block;flex:none}",
      // padding:0 is load bearing. The panel is a <section>, and the site
      // stylesheet carries section{padding-block:5.5rem}, which lands 88px of
      // padding top and bottom inside the panel and leaves the message stream
      // with almost nothing. Do not remove it.
      ".qx-panel{position:absolute;inset-block-end:60px;inset-inline-end:0;width:min(340px,calc(100vw - 32px));height:min(480px,calc(100vh - 140px));height:min(480px,calc(100dvh - 140px));padding:0;display:flex;flex-direction:column;background:#FFF9F2;border:1px solid #D7A347;border-radius:2px;box-shadow:0 24px 60px rgba(1,18,48,.22);overflow:hidden}",
      ...
      ".qx-quiet{opacity:.6}",
      ".qx-composer{display:flex;gap:10px;padding:14px 22px;border-top:1px solid rgba(1,18,48,.1);background:#F1E3D8}",
      ...
      // Gold fill is reserved for this one control: it is the primary action.
      // The disabled state carries no element opacity on purpose. Fading navy
      // over the ivory composer composites to a neutral grey, which is how the
      // old button ended up off palette; holding the fill at #011230 and dimming
      // only the text and the rule keeps every pixel inside the palette.
      ".qx-composer button{padding:0 18px;font:inherit;font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#011230;background:#D7A347;border:1px solid #D7A347;border-radius:2px;cursor:pointer;transition:background-color .2s ease,color .2s ease,border-color .2s ease}",
      ".qx-composer button[disabled]{background:#011230;color:rgba(241,227,216,.55);border-color:rgba(215,163,71,.45);cursor:default}",
      ".qx-khaled :focus-visible{outline:2px solid #D7A347;outline-offset:2px}",
      ".qx-foot{padding:10px 22px;font-size:12px;line-height:1.55;text-align:center;background:#F1E3D8;border-top:1px solid rgba(1,18,48,.08);opacity:.72}",
