 ...
 * Path in repository: /assets/js/professionals-directory.v5.js
 *
...
(function () {
  ...
  var root, list, count, searchInput, roleSelect, emirateSelect, specSelect, clearButton;
  var roleOptions = [], emirateOptions = [], specOptions = [];
  ...
  function buildControls() {
    ...
    var roleField = el("div", "qx-dir-field");
    var roleLabel = el("label", "qx-dir-label", t("role_label"));
    roleSelect = el("select", "field");
    roleSelect.id = "qx-dir-role";
  ...
  /** A scalar field, e.g. title_ar and title_en, joined for a containment test. */
  function recordText(record, base) {
    var parts = [];
    for (var i = 0; i < RECORD_LANGS.length; i++) {
      var value = record[base + "_" + RECORD_LANGS[i]];
      if (typeof value === "string") parts.push(norm(value));
    }
    return parts.join(" ");
  }

  /**
   * A role is matched on the keyword, not on the whole title. Titles are written
   * freely, so "Independent Real-Estate Broker" and "وسيط عقاري مستقل" both
   * belong under broker although neither equals the option's label. The keywords
   * live in the locale file beside the label, never here.
   */
  function keywordMatches(record, base, options, selected) {
    if (selected === "") return true;
    var option = options[Number(selected)];
    if (!option) return true;
    var text = recordText(record, base);
    for (var i = 0; i < option.match.length; i++) {
      if (option.match[i] && text.indexOf(option.match[i]) !== -1) return true;
    }
    return false;
  }
  ...
  function matches(record) {
    ...
    if (!keywordMatches(record, "title", roleOptions, roleSelect.value)) return false;
  ...
  function narrowed() {
    return !!(searchInput.value.trim() || roleSelect.value || emirateSelect.value || specSelect.value);
  }
