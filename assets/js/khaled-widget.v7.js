 ...
 * Path in repository: /assets/js/khaled-widget.v7.js
 *
 * Framework free. Add one line before </body> in every page layout:
 *   <script defer src="/assets/js/khaled-widget.v7.js"></script>
 *
...
(function () {
  ...
  function injectStyles() {
    var css = [
      ...
      // text is the whole affordance, and one color property drives the label
      // and the mark together, since the mark below follows currentColor.
      ...
      // Scoped to the launcher, so the 30px mark in the panel header keeps its
      // gold hexagon and ivory nodes. CSS beats the presentation attributes.
      ".qx-launcher .qx-mark path{stroke:currentColor}",
      ".qx-launcher .qx-mark circle{fill:currentColor}",
      ...
      ".qx-panel{position:absolute;inset-block-end:60px;inset-inline-end:0;width:min(340px,calc(100vw - 32px));height:min(480px,calc(100vh - 140px));height:min(480px,calc(100dvh - 140px));padding:0;display:flex;flex-direction:column;background:#FFF9F2;border:1px solid rgba(1,18,48,.12);border-top:2px solid #D7A347;border-radius:2px;box-shadow:0 24px 60px rgba(1,18,48,.22);overflow:hidden}",
